...
async def get_contacts(skip: int, limit: int, user: User, db: Session) -> List[Contact]:
    return db.query(Contact).filter(Contact.id == user.id).offset(skip).limit(limit).all()
...
async def get_contact(contact_id: int, user: User, db: Session) -> Contact:
    return db.query(Contact).filter(Contact.id == user.id).filter(and_(Contact.id == contact_id)).first()
